-- O token do cron viaja em header próprio, não no bearer.
--
-- Correção da 20260917180000: mandar DISPATCH_TOKEN como bearer não passa,
-- porque o gateway roda verify_jwt antes da função e recusa o que não tem
-- formato de JWT (UNAUTHORIZED_INVALID_JWT_FORMAT).
--
-- Agora o bearer volta a ser a service role JWT — que satisfaz o gateway — e a
-- prova de que a chamada veio do cron vai em x-dispatch-token, conferido
...
BEGIN
  ...
  BEGIN
    ...
    _dispatch_tok := public.app_setting('app.dispatch_token');
    ...
    BEGIN
      PERFORM net.http_post(
        ...
        headers := jsonb_build_object(
          ...
          'x-dispatch-token', coalesce(_dispatch_tok, '')
        ),
