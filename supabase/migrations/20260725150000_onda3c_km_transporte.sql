-- Onda 3c — Veículo próprio: reembolso por quilometragem (R$ 1,20/km)
--
-- Política de Reembolso O2 (PO-0002) 4.4.6: veículo particular em atividades
-- externas = R$ 1,20 por km rodado (já contempla combustível, manutenção, seguro).
-- O percurso e a finalidade devem constar no relatório.
--
-- Modelo:
--  - expenses.distance_km            — km rodados (quando reembolso por km).
--  - expense_policies.km_rate_cents  — tarifa por km (default 120 = R$ 1,20), parametrizável.
--  O valor (amount_cents) é calculado no cliente como round(distance_km * km_rate_cents)
--  e travado, para o usuário não digitar um valor divergente.
...
ALTER TABLE public.expense_policies
  ADD COLUMN IF NOT EXISTS km_rate_cents integer NOT NULL DEFAULT 120;

-- RPC passa a persistir distance_km (assinatura muda → DROP + CREATE).
DROP FUNCTION IF EXISTS public.create_expense_in_current_report(
  text, integer, date, uuid, uuid, uuid, text, text, boolean, text, text, boolean
);
